            ...
            {!!
             Table::withContents($books->items())->striped()
                ->callback('Ações',function ($field,$book){
                 ...
                 $retoreForm = "restore-form-{$book->id}";
                 $form = Form::open(['route' =>['trashed.books.update','books'=>$book->id],
                                    'method'=>'PUT','id'=>$retoreForm,'style'=>'display:none']).
                                     Form::hidden('redirect_to',URL::previous()).
                                     Form::close();
                 $anchorRestore = Button::link('Restaurar')
                                          ->asLinkTo($linkDestroy)->addAttributes([
                                           'onclick'=>"event.preventDefault();document.getElementById(\"{$retoreForm}\").submit()"
                                          ]);
